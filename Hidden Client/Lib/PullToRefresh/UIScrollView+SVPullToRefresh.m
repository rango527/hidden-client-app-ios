...
- (void)layoutForHorizontal {
    ...
    CGFloat marginY = 2;
    CGFloat labelMaxWidth = 60;

    self.titleLabel.text = [self.titles objectAtIndex:self.state];
    self.titleLabel.hidden = self.scrollView.contentOffset.x > -80;
    ...
    CGFloat labelX = (self.frame.size.width - titleSize.width) / 2 - 5.0f;
    if(subtitleSize.height > 0){
...
- (void)resetScrollViewContentInset {
    ...
    switch (self.position) {
        ...
        case SVPullToRefreshPositionBottom:
            ...
            break;
        case SVPullToRefreshPositionLeft:
            currentInsets.left = self.edgeOffset;
            break;
        case SVPullToRefreshPositionRight:
    ...
    if(self.state != SVPullToRefreshStateLoading) {
        ...
        switch (self.position) {
            ...
            case SVPullToRefreshPositionBottom:
                ...
                break;
            case SVPullToRefreshPositionLeft:
                [self.titleLabel setHidden: contentOffset.x > -80];
                
                scrollOffsetThreshold = -UIScreen.mainScreen.bounds.size.width * 0.35;
                break;
            case SVPullToRefreshPositionRight:
...
- (UILabel *)titleLabel {
    if(!_titleLabel) {
        _titleLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, 240, 20)];
        _titleLabel.text = NSLocalizedString(@"Pull to refresh",);
        _titleLabel.font = [UIFont fontWithName:@"Avenir-Book" size:14.0];
    ...
    switch (self.position) {
        ...
        case SVPullToRefreshPositionLeft:
            if(fequalzero(self.scrollView.contentOffset.x)) {
                [self.scrollView setContentOffset:CGPointMake(-self.frame.size.width, 0.0f) animated:YES];
                self.wasTriggeredByUser = NO;
            }
            else
                self.wasTriggeredByUser = YES;
...
- (void)stopAnimating {
    ...
    switch (self.position) {
        ...
        case SVPullToRefreshPositionLeft:
            if(!self.wasTriggeredByUser)
                [self.scrollView setContentOffset:CGPointMake(-self.edgeOffset, self.scrollView.contentOffset.y) animated:YES];
            break;
        case SVPullToRefreshPositionRight:
            if(!self.wasTriggeredByUser)
