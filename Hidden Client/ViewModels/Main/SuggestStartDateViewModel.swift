//
//  SuggestStartDateViewModel.swift
//  Hidden Client
//
...
class SuggestStartDateViewModel: RootVM {
    ...
    func suggestStartDate(with text: String, _ completion: (()->())? = nil) {
        guard let id = process.id else { return }
        
        DispatchQueue.main.async { HUD.show(.progress) }
        APIManager.getObject(.suggestStartDate(id, text)).subscribe(onNext: { (item: Model) in
